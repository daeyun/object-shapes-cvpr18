...
DEFINE_string(render_for_cnn_dir, "", "");
DEFINE_string(database_name, "shrec12", ".sqlite suffix will be added.");
...
void RenderImages(const vector<const mv::Rendering *> &renderables) {
  ...
  LOG(INFO) << "Forking into " << FLAGS_num_processes << " processes.";
  ...
  for (int i = start_end.first; i < start_end.second; ++i) {
    ...
    if (renderable->rendering_type() == mv::Rendering_Type_NORMAL) {
    ...
    if (FileIO::Exists(out_filename)) {
      LOG_EVERY_N(INFO, 100000) << "Already exists: " << out_filename;
      continue;
    }

    LOG_EVERY_N(INFO, 100000) << Data::ToString(*renderable);
    timer.Toc();

    string mesh_filename = FileIO::FullDataPath(renderable->mesh_filename());

    // TODO(daeyun): fix hack
    // The name of the dataset is not explicitly stored in the rendering proto, so check the mesh filename and check if it contains the name.
    bool is_shapenetcore = mesh_filename.find("/shapenetcore") != std::string::npos;
    ...
    if (renderable->rendering_type() == mv::Rendering_Type_RGB) {
      // TODO: RGB is only implemented for shapenetcore.
      if (is_shapenetcore && renderable->set_size() == 1) {
//        LOG(INFO) << out_filename;
        if (!FileIO::Exists(out_filename)) {
          // Should have been generated by the python script. Just make sure it exists, no need to render.
//          LOG(ERROR) << "File does not exist: " << out_filename;
          // TODO(daeyun): Disable this for now. Once this program is finalized, it should be turned back on.
//          throw std::runtime_error("");
        }
      }
      continue;
...
//    Expects(std::abs(15 - (camera->position() - camera->lookat_position()).norm()) < 1e-2);
    ...
    } else if (renderable->set_size() == 20) {
      //TODO(daeyun)
      // If multiple images is requested, derive the cameras from the reference camera.
      // The reference camera does not have to be part of the 6.
      SixViews(*camera, &cams);
...
//void RenderImages(const vector<const mv::Rendering *> &renderables) {
void Voxelize(const vector<const mv::Rendering *> &renderables) {
  auto sorted_renderables = vector<const mv::Rendering *>(renderables.begin(), renderables.end());

  // Don't assume the outputs are generated in the same order as the depths.
  // Sorted by mesh filenames to improve cache behavior.
  std::sort(sorted_renderables.begin(),
            sorted_renderables.end(),
            [](const mv::Rendering *a, const mv::Rendering *b) -> bool {
              return a->mesh_filename() < b->mesh_filename();
            });

  typedef std::tuple<std::vector<std::array<int, 3>>, std::vector<std::array<float, 3>>> FaceVertex;

  cache::lru_cache<string, shared_ptr<FaceVertex>> mesh_cache(500);
  auto read_mesh = [&](const string &filename) {
    if (!mesh_cache.exists(filename)) {
      auto fv = make_shared<FaceVertex>();
      LOG(INFO) << "Reading mesh: " << filename;
      bool ok = FileIO::ReadFacesAndVertices(filename, &std::get<0>(*fv), &std::get<1>(*fv));
      ...
      mesh_cache.put(filename, fv);
    }
    return mesh_cache.get(filename);
  };
  string prev_mesh_filename;

  Timer timer("rendering voxels");
  ...
  for (int i = start_end.first; i < start_end.second; ++i) {
    const mv::Rendering *renderable = sorted_renderables[i];
    Expects(renderable->rendering_type() == mv::Rendering_Type_VOXELS);
    ...
    // Saves disk IO.
    auto fv_mesh = read_mesh(mesh_filename);

    auto rotmat = camera->view_mat().topLeftCorner<3, 3>();

    const std::vector<std::array<float, 3>> &vertices = std::get<1>(*fv_mesh);
    // Copy vertex data and perform camera transformation.
    std::vector<std::array<float, 3>> vertices_cam = vertices;
    for (auto &vertex : vertices_cam) {
      Vec3 out = rotmat * Vec3 {vertex[0], vertex[1], vertex[2]};
      vertex[0] = out[0];
      vertex[1] = out[1];
      vertex[2] = out[2];
    }
    ...
    constexpr float rad = 1.0;

    std::array<float, 3> bmin = {-rad, -rad, -rad};
    std::array<float, 3> bmax = {rad, rad, rad};

    auto voxel_grid = voxel::VoxelGrid(resolution, resolution, resolution);
    voxel::Voxelize(std::get<0>(*fv_mesh), vertices_cam, resolution, kPadding, scale, bmin, bmax, &voxel_grid);

    LOG(INFO) << "Saving " << out_filename;
    FileIO::PrepareDir(fs::path(out_filename).parent_path().string());
    voxel_grid.save(out_filename);
...
//  int count = Data::ReadRenderables("database/shapenetcore.sqlite", "all_renderings", &renderables);
//  int count = Data::ReadRenderables("database/shapenetcore_subset.sqlite", "all_renderings", &renderables);
  int count = Data::ReadRenderables("database/" + FLAGS_database_name + ".sqlite", "all_renderings", &renderables);
...
int SaveExamples(const string &sqlite_file, const string &view_name, const string &out_dir) {
  ...
  LOG(INFO) << "serializing json into CBOR";
  std::vector<uint8_t> serialized = json::to_cbor(data);
  LOG(INFO) << "serialized string size: " << serialized.size()/ static_cast<float>(1024*1024) << " MB";

#if 0
  LOG(INFO) << "compressing serialized string";
  string compressed;
  mvshape::FileIO::CompressBytes(serialized.data(), serialized.size(), "lz4hc", 7, sizeof(char), &compressed);
  LOG(INFO) << "compression ratio: " << static_cast<float>(serialized.size()) / compressed.size();
#endif

  string filename = boost::filesystem::absolute(fs::path(FileIO::FullOutPath(out_dir)) / "all_examples" ).string() + ".cbor";
  FileIO::PrepareDir(fs::path(filename).parent_path().string());
  LOG(INFO) << "Saving " << filename;

  std::ofstream file;
  file.open(filename, std::ios_base::out | std::ios_base::binary);
  mvshape::FileIO::WriteBytes(serialized, &file);
  file.close();
...
void LoadRenderingContent(const mv::Rendering &rendering, string *data) {
  ...
  switch (rendering.rendering_type()) {
    case mv::Rendering_Type_DEPTH:Ensures(data->size() == 4 * r * r * v);
      break;
    case mv::Rendering_Type_NORMAL:
    case mv::Rendering_Type_RGB:Ensures(data->size() == 4 * r * r * v * 3);
      break;
    case mv::Rendering_Type_VOXELS:Ensures(data->size() == r * r * r * v);
      break;
    default:LOG(ERROR) << "Unknown rendering type: " << rendering.rendering_type();
...
int ReadSquareRgbImage(const std::string &filename, std::vector<uint8_t> *data) {
  auto img = cv::imread(filename, CV_LOAD_IMAGE_COLOR);
  if (img.size[0] != img.size[1]) {
    LOG(ERROR) << "Image is not square: " << filename;
    throw std::runtime_error("");
  }
  Expects(img.elemSize() == 3);  // sizeof(uchar) * 3
  data->resize(img.elemSize() * img.size[0] * img.size[1]);
  std::memcpy(data->data(), img.data, data->size());
  return img.size[0];
}
